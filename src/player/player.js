...
export function createPlayer({ mount, board: raw, baseUrl = '', resolveSrc, initialCam = null }) {
  ...
  // The step docking makes current for a screen: the first step that frames it,
  // or — for a screen no step points at yet — the group's opening step, so
  // docking always lands somewhere playable. (The click affordance uses the
  // stricter core stepForScreen(), which returns null when no step frames it.)
  function dockStep(screenId) {
    for (const g of board.groups) {
